...
class IndexFile {
    ...
    checkFileExists = async () => {
        try {
            ...
            const headS3Params = this.helpers.buildS3HeadParams(Constants.BUCKETNAME, `${Constants.BUCKETKEY}/${this.fileName}`);
    ...
    saveURLtoDB = async (isSuccess, error) => {
        ...
        if(isSuccess){
            ...
            const snsParams = this.helpers.buildSNSParams("test", `Failed to download index file for ${year}, ${quarter}`);
